...
[Download the latest Android APK](https://github.com/aussiewaska-coder/OpenStrike/releases/download/weather-effects-2026-09-09/OpenStrike-shockwave-diffuse-smoke-2026-09-09.apk) · ARM64 debug build, with the latest weather and strike effects.
...
Sun position can follow the theatre's location, date, and time, or use fixed lighting presets including sunrise and noon. A broad horizon haze blends distant terrain into the sky. **Options → Display → Weather** cycles through Clear, Overcast, Rain, and Storm; **Time of day** is beside it. Weather transitions include rain, wet surfaces, wind, and lightning, rather than a live weather feed.

Clouds now have distinct layers: thin high cirrus, lower cumulus banks with varied altitude, thickness, and density, and rising cumulonimbus with broad tops in storm weather. Lower clouds use depth-clipped volumetric extinction; the high cirrus is an inexpensive background layer. The mobile default keeps 48 view samples over 8 km, simple lighting, and reduced 3D resolution. These are stylized, performance-conscious effects rather than a meteorological simulation. See [volumetric weather](docs/volumetric-weather.md) for controls and rendering limits.

| Sunrise: cirrus above the banks | Storm cloud towers |
| --- | --- |
| ![Sunrise cirrus above lower clouds, test-scene render](docs/media/weather-sunrise-layers.png) | ![Storm towers below high cirrus, test-scene render](docs/media/weather-storm-towers.png) |

*Actual Compatibility-renderer test scenes; phone frame rate has not been measured for the latest effects.*
...
Bombs, missiles, rockets, and destroyed targets produce larger explosions, expanding shockwaves, and sustained flames with rising black smoke. Nearby repeat strikes renew fires, building strikes produce larger and longer-lasting plumes, and destroyed aircraft leave falling fire sources. Smoke grows and drifts with the wind, then becomes softer, lighter, and more diffuse as it rises above the source. Water surface impacts use splash effects.

Smoke uses shaded particles to suggest volume without another ray-marched pass. Fixed pools limit the number of fires and shockwaves. Building damage is tracked without a full structural-collapse simulation. Target tracking remains aimed at the same contact when changing aircraft camera views. See [strike effects and tracking](docs/strike-fire-and-tracking.md) for implementation and validation details.

| Rising smoke disperses with height | Expanding explosion shockwave |
| --- | --- |
| ![Black smoke becoming diffuse above burning targets, test-scene render](docs/media/strike-diffuse-smoke.png) | ![Explosion with an expanding pressure-wave shell, test-scene render](docs/media/strike-shockwave.png) |
