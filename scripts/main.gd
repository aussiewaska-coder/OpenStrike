...
## Field of view opens with speed. The eye reads widening as accelerating even
## when the number on the HUD has stopped moving.
@export var jet_base_fov := 62.0
@export var jet_fov_gain := 22.0
...
func _apply_follow_camera(delta: float, snap: bool = false) -> void:
	...
	if not _free_look.is_zero_approx():
		...
		if camera.global_position.distance_squared_to(_look_target) > 0.0001:
			camera.look_at(_look_target, _camera_horizon(0.0, true))
