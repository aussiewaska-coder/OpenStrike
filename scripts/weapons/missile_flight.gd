...
# Defaults preserve player weapons; hostile rounds use their own tuning.
var turn_limit_g := 28.0
var maximum_turn_rate := 2.2
var seeker_cone_degrees := 80.0
var maximum_speed := 950.0
var maximum_lead_seconds := 4.0
var motor_acceleration_scale := 1.0
var fuse_radius := FUSE_RADIUS
var loss_tolerance := 0.65
...
func tracking_state(point: Vector3, velocity: Vector3) -> Dictionary:
	var c := target()
	var valid := not c.is_empty() and TRACKER.is_airborne(c) and not _guidance_lost
	if valid and seeker == Seeker.RADAR:
		valid = radar_lock_provider.is_valid() and int(radar_lock_provider.call()) == target_handle
	if valid and velocity.length_squared() > 1.0:
		valid = velocity.angle_to(c.position - point) < deg_to_rad(seeker_cone_degrees)
	return {
		"weapon": "RADAR MISSILE" if seeker == Seeker.RADAR else "HEAT SEEKER",
		"target_position": c.get("position"), "target_name": c.get("name", "TARGET"),
		"locked": valid, "status": "TARGET LOST" if c.is_empty() else ("LOCKED" if valid else "LOCK LOST"),
	}
...
func advance(point: Vector3, velocity: Vector3, delta: float, age := 0.0) -> Array:
	...
	if valid and velocity.length_squared() > 1.0:
		...
		valid = velocity.normalized().angle_to(offset.normalized()) < deg_to_rad(seeker_cone_degrees)
	_seeker_lost_time = 0.0 if valid else _seeker_lost_time + delta
	if _seeker_lost_time > loss_tolerance:
		_guidance_lost = true
	_last_target = c if valid and not _guidance_lost else {}
	if age >= IGNITION_DELAY and not _last_target.is_empty():
		var offset: Vector3 = c.position - point
		var lead_time := clampf(offset.length() / maxf(velocity.length(), 250.0), 0.0, maximum_lead_seconds)
		...
		var max_rate := minf(maximum_turn_rate, turn_limit_g * GRAVITY / maxf(next.length(), 100.0))
		if angle > 0.00001:
			heading = heading.slerp(wanted, minf(1.0, max_rate * delta / angle)).normalized()
		next = heading * next.length()
	if is_boosting(age):
		next += next.normalized() * (155.0 if seeker == Seeker.RADAR else 190.0) * motor_acceleration_scale * delta
	next.y -= GRAVITY * delta
	next = next.limit_length(maximum_speed)
