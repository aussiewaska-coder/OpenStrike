...
## Field of view widens with speed. The eye reads widening as accelerating even
## when the number on the HUD is not moving, which is why every game that wants
## speed to feel like speed does this.
static func field_of_view(
	...
	return base_fov + fraction * fov_gain
