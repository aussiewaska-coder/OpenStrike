...
func _run() -> void:
	...
	assert(field.launcher_count() == 10, "the field must instantiate ten launchers")
	assert(field.hit_index.entity_count() == 10, "every launcher must have projectile collision")
	var markers := field.find_children("ArcadeTargetRing", "MeshInstance3D", true, false)
	assert(markers.size() == 10, "every launcher must have one arcade target ring")
	...
	var first := LAYOUT.positions_for("au_qld_surfers")[0]
	var hit: RefCounted = field.query_segment(
		Vector3(first.x, 30.0, first.y),
		Vector3(first.x, -10.0, first.y)
	)
	...
	assert(field.launcher_count() == 9, "one hit must remove exactly one launcher")
	assert(field.hit_index.entity_count() == 9, "destroyed launchers must stop blocking projectiles")
