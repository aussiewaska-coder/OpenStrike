...
func _run() -> void:
	...
	var jet := JET.new()
	root.add_child(jet)
	assert(jet.airframe.cockpit_seat_fraction >= 0.58, "pilot eye must sit forward near the HUD")
	assert(jet.airframe.cockpit_eye_height_fraction >= 0.78, "pilot eye must sit high enough to see the panel")
