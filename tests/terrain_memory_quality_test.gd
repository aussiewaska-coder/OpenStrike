...
func run():
	...
	var pixels: int = terrain.near_detail_texture_px ** 2 + (terrain.max_detail_chunks - 1) * terrain.detail_texture_px ** 2 + terrain.far_detail_chunks * terrain.far_detail_texture_px ** 2
	assert(pixels <= 14 * 2048 ** 2, "sharp nearby imagery must fit the uncompressed resident budget")
	terrain.free()
