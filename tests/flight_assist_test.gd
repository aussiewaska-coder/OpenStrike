...
func _bank_hold() -> void:
	...
	_assert_approx(
		assist.commanded_roll_rate(1.0, 1.8, aero.airframe.corner_speed_mps(), 0.0, 0.0),
		1.8,
...
func _coordination_envelope() -> void:
	...
	var ceiling := acos(1.0 / aero.airframe.load_limit_g)
	...
	for speed in [90.0, 110.0, 155.0, 260.0]:
		...
		var available: float = minf(aero.airframe.load_limit_g, aero.aerodynamic_load_limit(speed))
...
func _thrust_vectoring() -> void:
	var stall := deg_to_rad(aero.airframe.stall_alpha_degrees)
	var post := deg_to_rad(aero.airframe.post_stall_alpha_degrees)
	...
	if post_stall_lift >= aero.airframe.cl_max():
